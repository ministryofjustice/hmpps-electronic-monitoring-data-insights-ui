...
export type StubViolations200Options = {
  status: 200
  consumerId: string
  response: {
...
const defaultStubViolationsOptions: StubViolations200Options = {
  status: 200,
  consumerId: '1',
  response: {
...
const stubViolationsEmpty = (options: StubViolations200Options = defaultStubViolationsOptions): SuperAgentRequest =>
  stubFor({
    request: {
      method: 'GET',
      urlPattern: `/people/${options.consumerId}/curfew/violations`,
    },
    response: {
...
export default stubViolationsEmpty
