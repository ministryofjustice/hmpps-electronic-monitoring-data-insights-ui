...
import { initialiseTechnicalUpdatesBanner, hideBannerIfAlreadySeen } from './technicalUpdatesBanner'
...
describe('initialiseTechnicalUpdatesBanner', () => {
  ...
  describe('hideBannerIfAlreadySeen', () => {
    it('hides the banner when the version has already been seen', () => {
      localStorage.setItem(STORAGE_KEY, '0.0.1')
      renderBanner('0.0.1')
      hideBannerIfAlreadySeen()

      const banner = document.getElementById('technical-updates-banner') as HTMLElement
      expect(banner.style.display).toBe('none')
    })

    it('does not hide the banner when the version has not been seen', () => {
      renderBanner('0.0.1')
