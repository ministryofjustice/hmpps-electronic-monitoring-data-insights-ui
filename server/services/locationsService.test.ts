...
describe('LocationsService', () => {
  ...
  describe('getLocations', () => {
    it('maps the API locations response into domain locations', async () => {
      ...
      const result = await locationsService.getLocations(token, personId, from, to, nextToken)

      expect(result).toEqual<LocationsResult>({
        locations: [mappedLocation],
        nextToken,
      })
      expect(locationsApiClient.getLocations).toHaveBeenCalledWith(token, personId, from, to, nextToken)
    })
  })
  ...
  describe('getLocation', () => {
    it('maps the API single location response into domain locations', async () => {
      ...
      const result = await locationsService.getLocation(token, personId, positionId)

      expect(result).toEqual([mappedLocation])
      expect(locationsApiClient.getLocation).toHaveBeenCalledWith(token, personId, positionId)
    })
  })
})
