...
const searchLocationsQueryValidationSchema = z
  ...
  .refine(data => data.start.date.length < 8 || parseDateTimeFromComponents(data.start.date, '0', '0').isValid(), {
    message: 'You must enter a valid From date and time',
    path: ['start', 'date'],
  })
  .refine(data => data.end.date.length < 8 || parseDateTimeFromComponents(data.end.date, '0', '0').isValid(), {
