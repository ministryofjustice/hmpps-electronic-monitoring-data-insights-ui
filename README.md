...
Our security policy is located [here](https://github.com/ministryofjustice/hmpps-electronic-monitoring-data-insights-ui/security/policy).
...
is the usual way that projects are set up. This namespace includes an AWS elasticache setup - which is required by this
template project.
...
in the Cloud Platform user guide. Once that is set up then the `custom severity label` can be used for
